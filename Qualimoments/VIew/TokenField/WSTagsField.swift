...
extension WSTagsField {
    ...
    fileprivate func enumerateItemRects(layoutWidth: CGFloat, using closure: (_ tagView: WSTagView?, _ tagRect: CGRect?, _ textFieldRect: CGRect?) -> Void) {
        ...
        var totalHeight: CGFloat = WSConstants.STANDARD_ROW_HEIGHT
        ...
        for tagView in tagViews {
            ...
            if curX + tagRect.width > maxWidth {
                // Need a new line
                curX = 0
                curY += WSConstants.STANDARD_ROW_HEIGHT + spaceBetweenLines
                totalHeight += WSConstants.STANDARD_ROW_HEIGHT
            }

            tagRect.origin.x = curX
            // Center our tagView vertically within STANDARD_ROW_HEIGHT
            tagRect.origin.y = curY + ((WSConstants.STANDARD_ROW_HEIGHT - tagRect.height)/2.0)
        ...
        curX += max(0, WSConstants.TEXT_FIELD_HSPACE - self.spaceBetweenTags)
        var availableWidthForTextField: CGFloat = maxWidth - curX

        if textField.isEnabled {
            var textFieldRect = CGRect.zero
            textFieldRect.size.height = WSConstants.STANDARD_ROW_HEIGHT

            if availableWidthForTextField < WSConstants.MINIMUM_TEXTFIELD_WIDTH {
                ...
                curX = 0 + WSConstants.TEXT_FIELD_HSPACE
                curY += WSConstants.STANDARD_ROW_HEIGHT + spaceBetweenLines
                totalHeight += WSConstants.STANDARD_ROW_HEIGHT
                // Adjust the width
...
//            self.scrollRectToVisible(textField.frame, animated: false)
            self.setContentOffset(CGPoint(x: self.contentOffset.x, y: 0), animated: true)
        }
    }
    ...
    private var maxHeightBasedOnNumberOfLines: CGFloat {
        ...
        return contentInset.top + contentInset.bottom + WSConstants.STANDARD_ROW_HEIGHT * CGFloat(numberOfLines) + spaceBetweenLines * CGFloat(numberOfLines - 1)
    }
